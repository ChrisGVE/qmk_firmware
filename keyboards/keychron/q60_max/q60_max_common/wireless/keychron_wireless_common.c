...
void wireless_pre_task(void) {
    ...
    if (time == 0) {
        if ((gpio_read_pin(BT_MODE_SELECT_PIN) << 1 | gpio_read_pin(P2P4_MODE_SELECT_PIN)) != mode) {
            mode = gpio_read_pin(BT_MODE_SELECT_PIN) << 1 | gpio_read_pin(P2P4_MODE_SELECT_PIN);
            time = timer_read32();
        }
    }
